        ...
        if (messageText && messageText[0]=='/' && message.name!=process.env.BOT_NAME) {
            if (linkRegex.test(messageText)) {
                ...
                if (beer && beer.length >= 2) {
                    ...
                    Records.updateBeerSheet(beer[1], beer[2], date, image);
                ...
                if(tournament && tournament.length >= 6) {
                    let date = Date.now().toLocaleString();
                    Records.updateTournamentsSheet(date, tournament[1], tournament[2], tournament[3], tournament[4], tournament[5]);
                ...
                if(event && event.length >= 6) {
                    let date = Date.now().toLocaleString();
                    Records.updateChainbreakerEventsSheet(date, event[1], event[2], event[3], event[4], event[5]);
