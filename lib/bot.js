...
class Bot {
    ...
    static checkMessage(message) {
        ...
        const beerParseBotRegex = /^\/beer\s+(.*?)\s(.*?)\s(.*)/;
        ...
        if (messageText && workoutBotRegex.test(messageText)  && message.name!=process.env.BOT_NAME) {
            if (linkRegex.test(messageText)) {
                ...
                if (beer && beer.length == 3) {
                    let date = Date.now().toLocaleString();
                    Records.updateBeerSheet(beer[0], beer[1], date, beer[3]);
                    return '' + beer[0] + ' owes ' + beer[1] + ' a beer ' + beer[3];
                }
                else {
                ...
                var event = eventLogRegex.exec(messageText);
                ...
                if(note && note.length == 1) {
                    let date = Date.now().toLocaleString();
                    Records.updateNotesSheet(date,message.name,note[0]);
